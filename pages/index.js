...
export default function Home() {
	...
	useEffect(() => {
		if (body)
			body.current.addEventListener("mousemove", function (e) {
				...
				if (dt > 100) {
					...
					const playbackSpeed = Math.abs(Math.round(dx / dt * 100)) + Math.abs(Math.round(dy / dt * 100))
					if (playbackSpeed < 50) {
						setPlaybackRate(0.3)
		...
		return () => {
			if (body)
				body.current?.removeEventListener('mousemove', () => console.log('abort'));
		};
	}, [body]);
	useEffect(() => {
		videoRef.current.playbackRate = playbackRate;
		const timer = setTimeout(() => {
			if (videoRef.current.playbackRate > 0.3)
				setPlaybackRate(0.3)
		}, 5000);
	...
	return (
		<React.Fragment>
			<Container style={{ height: "100vh" }} fluid ref={body}>
				<CustomNavbar />
				<Row>
					<Col lg={{ span: 6, offset: 3 }} className="d-flex justify-content-center">
						<video ref={videoRef} autoPlay loop className="home-images" width="600"
							height="600">
